...
bool Directory::insert(Node*& node, string uName, string fName, string lName, string mName, int numW, int numL)
{
	if (node == nullptr)
	{
		node = new Node(uName, fName, lName, mName, numW, numL);
		size++;
		return true;
	}
	else if (uName > node->uName)
	{
		return insert(node->right, uName, fName, lName, mName, numW, numL);
	}
	else if (uName < node->uName)
	{
		return insert(node->left, uName, fName, lName, mName, numW, numL);
	}
	return false;
}
