...
* Program Description:
* The Main file of execution for the usage of my DirectoryAVL class.
* Borrowed and edited from Professor Knight's original Tokenizer example class.
* Controls the flow of the program through the tokenizer and DirectoryAVL classes
*/
...
bool processText(istream& is, bool usingFile = false)
{
	...
	// directory order ... displays the current directory in the given order (in, pre, or post)
	...
	while (true)
	{
		...
		else if (command == "load")
		{
			...
			if (!tkn.readWord(token1)) {
				cout << "Missing file name or \"from\"\n";
				continue; // skip back to the top of the read loop
			}
			bool from = token1 == "from";
			if (from)
			{
				//overwriting token1 if "from" branch because we no longer need "from"
				if(!tkn.readWord(token1))
				{
		...
		else if (command == "directory")
		{
			if(tkn.readWord(token1))
			{
				...
				{
					cout << "Expected string \"in\", \"pre\", or \"post\" for <order>\n"; // bad
				}
			}
			else
			{
